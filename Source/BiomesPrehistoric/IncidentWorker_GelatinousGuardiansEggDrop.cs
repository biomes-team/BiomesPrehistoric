      ...
      new Pair<int, float>(4, 0.4f)
        ...
        private void SpawnGelatinousRaptorEgg(IntVec3 firstEggPos, Map map, int count)
        {
            ...
            for (int index = 0; index < count - 1; ++index)
            {
                ...
                FilthMaker.TryMakeFilth(pos, map, BiomesPrehistoric_DefOf.Filth_GelatinousSlime);
            }
        }
        private void GelatinousRaptorEgg(IntVec3 pos, Map map) => SkyfallerMaker.SpawnSkyfaller(BiomesPrehistoric_DefOf.BMT_GelatinousRaptorEggIncoming, BiomesPrehistoric_DefOf.BMT_GelatinousRaptorEgg, pos, map);
        private bool TryFindGelatinousRaptorEggDropCell(IntVec3 nearLoc, Map map, int maxDist, out IntVec3 pos) => CellFinderLoose.TryFindSkyfallerCell(BiomesPrehistoric_DefOf.BMT_GelatinousRaptorEggIncoming, map, out pos, nearLoc: nearLoc, nearLocMaxDist: maxDist);
    }
}*/